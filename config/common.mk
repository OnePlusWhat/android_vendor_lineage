...
ifeq ($(LINEAGE_BUILDTYPE), RELEASE)
    ...
    ifeq ($(LINEAGE_VERSION_MAINTENANCE),0)
        ifeq ($(LINEAGE_VERSION_APPEND_TIME_OF_DAY),true)
            LINEAGE_VERSION := $(PRODUCT_VERSION_MAJOR).$(PRODUCT_VERSION_MINOR)-$(shell date -u +%Y%m%d_%H%M%S)-$(LOCAL_NLOS_VERSION)$(LINEAGE_EXTRAVERSION)-$(LOCAL_NLOS_BUILD)
        else
            LINEAGE_VERSION := $(PRODUCT_VERSION_MAJOR).$(PRODUCT_VERSION_MINOR)-$(shell date -u +%Y%m%d)-$(LOCAL_NLOS_VERSION)$(LINEAGE_EXTRAVERSION)-$(LOCAL_NLOS_BUILD)
        endif
    else
        ifeq ($(LINEAGE_VERSION_APPEND_TIME_OF_DAY),true)
            LINEAGE_VERSION := $(PRODUCT_VERSION_MAJOR).$(PRODUCT_VERSION_MINOR).$(LINEAGE_VERSION_MAINTENANCE)-$(shell date -u +%Y%m%d_%H%M%S)-$(LOCAL_NLOS_VERSION)$(LINEAGE_EXTRAVERSION)-$(LOCAL_NLOS_BUILD)
        else
            LINEAGE_VERSION := $(PRODUCT_VERSION_MAJOR).$(PRODUCT_VERSION_MINOR).$(LINEAGE_VERSION_MAINTENANCE)-$(shell date -u +%Y%m%d)-$(LOCAL_NLOS_VERSION)$(LINEAGE_EXTRAVERSION)-$(LOCAL_NLOS_BUILD)
        endif
    endif
endif
